...
export function ShopDisabledPanel({
  onClose,
  onInstall,
  installing = false,
  progressPercent = 0,
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 p-6">
      <div className="w-full max-w-sm rounded-3xl border border-slate-700/40 bg-slate-950/95 p-8 text-center shadow-2xl">
        ...
        <h2 className="mt-1 text-xl font-semibold text-white">Shop Skill Not Installed</h2>
        <p className="mt-3 text-sm leading-relaxed text-slate-400">
          Install the <span className="text-cyan-400">AMAZON-ORDERING</span> skill to let your
          agents walk to the office shop, manage shopping lists, and review Amazon order history.
        </p>
        ...
        {installing ? (
          <div className="mt-5 rounded-2xl border border-cyan-500/20 bg-cyan-500/5 p-4 text-left">
            ...
            <p className="mt-3 text-sm leading-relaxed text-slate-300">
              {progressMessage?.trim() || "Installing the amazon-ordering skill."}
            </p>
            <p className="mt-2 text-xs leading-relaxed text-slate-500">
              Once it&apos;s installed, Claw3D will unlock the office shop and shopping workflow.
            </p>
          </div>
        ) : null}
        ...
        <div className="mt-6 flex flex-col gap-3">
          ...
          >
            {installing
              ? "Installing AMAZON-ORDERING skill..."
              : "Install AMAZON-ORDERING skill"}
          </button>
          <button
            type="button"
